...
ADMINS = (
    ('marionette-todo administrator', 'donotreply@blitzagency.com'),
)
...
SECRET_KEY = r"6$-85zp62lz43q1+x36c(w9yd!vzh!fgm!=w7d#^l9-m6z&r4d"
...
LOGGING = {
    ...
    'filters': {
        ...
        'invalid_host_header': {
            '()': 'utils.invalid_host_header_filter.InvalidHostHeaderFilter'
        }
    },
    'formatters': {
        'verbose': {
            'format': '%(name)s %(levelname)s \
            %(asctime)s %(module)s %(process)d %(thread)d %(message)s'
        },
        'simple': {
    ...
    'loggers': {
        ...
        'z.pool': {
            'handlers': ENABLE_EMAIL_LOGGING and
            ['console', 'mail_admins'] or
            ['console'],
            'level': 'WARNING',
            'propagate': False,
        },
